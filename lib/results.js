...
//
// "Correct" here means: whichever side (home/draw/away) the single most
// confident published reading favored — the same rule the live site uses
// to pick its one headline "Match Winner" standout — matches the actual
// final result. This only grades the match-winner call, not Over/Under,
// Correct Score, or Handicap; those could be graded the same way later.
...
function predictedWinnerLabel(probs) {
  ...
  const best = readings.reduce((b, r) => (r.value > (b?.value ?? -1) ? r : b), null);
  return best.label; // "home" | "draw" | "away"
}
